...
try {
    ...
    if (board != null) {
        ...
        <div class='col-sm-10'>
        <input class='form-control' id='regDate' type='number' name='regDate' value='<%=board.getRegDate()%>'> 
        </div>
        </div>
        <div class='form-group row'>
        <label for='viewCount' class='col-sm-2 col-form-label'>조회수</label>
        <div class='col-sm-10'>
        <input class='form-control' id='viewCount' type='number' name='viewCount' value='<%=board.getViewCount()%>'> 
        </div>
        </div>
        <div class='form-group row'>
        <div class='col-sm-10'>
        <button class='btn btn-outline-info btn-sm'>변경</button>
        <a href='delete.jsp?no=<%=board.getNo()%>' class='btn btn-outline-info btn-sm'>삭제</a>
        </div>
        </div>
        </form>
<%
