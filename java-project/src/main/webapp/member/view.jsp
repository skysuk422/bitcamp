...
try {
    ...
    if (member != null) { %>
        ...
        <div class='col-sm-10'>
        <input class='form-control' id='createdDate' type='number' name='createdDate' value='<%=member.getCreatedDate()%>'> 
        </div>
        </div>
        <div class='form-group row'>
        <div class='col-sm-10'>
        <button class='btn btn-outline-info btn-sm'>변경</button>
        <a href='delete.jsp?no=<%=member.getNo() %>' class='btn btn-outline-info btn-sm'>삭제</a>
        </div>
        </div>
        </form>
<%
